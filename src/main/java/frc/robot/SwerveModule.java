...
public class SwerveModule {
  private static final double kWheelRadius = 0.308;
  ...
  public SwerveModule(
      ...
      //DriveEncoder no longer needs channel info, we pull from the CAN bus -DW
      //int driveEncoderChannelA,
      //int driveEncoderChannelB,
      int turningEncoderChannelA,
      int turningEncoderChannelB) {
    //Original Motor Definitions, based on PWMsparkmaxes -DW
    ...
    //11.17 Added to try to stop the wobs. Should be removed if it works without.
  ...
  public SwerveModuleState getState() {
    return new SwerveModuleState(m_driveEncoder.getVelocity(), new Rotation2d(m_turningEncoder.getDistance()));
  }
  ...
  public double getTurnOutput(SwerveModuleState desiredState) {
    ...
    return m_turningPIDController.calculate(m_turningEncoder.getDistance(), state.angle.getRadians());
  }
  /**
...
//I'm going to set the divemotor to 0, so we're just focused on the turn
    m_driveMotor.setVoltage(driveOutput + driveFeedforward);
    //m_driveMotor.setVoltage(0);
    //Removing the feedforward bit
    m_turningMotor.setVoltage(turnOutput + turnFeedforward);
    //m_turningMotor.setVoltage(turnFeedforward);
  }
}
